...
export const getCandidateByUserId = (setCandidate, userID) => {
    axios.get(`${baseURL}/candidates`)
    .then((response) => {setCandidate(response.data.find((candidate) => candidate.user.id === userID)); console.log(response.data.find((candidate) => candidate.user.id === userID))})
